...
When adding mocks or test utilities, apply the rules below to avoid common pitfalls.
...
Gate: Before mocking any method — ask "What side effects does the real method have?" and "Does this test depend on any of those?" Mock at the lowest level necessary, not "to be safe."
...
Testing is part of implementation, not optional follow-up. TDD cycle: write failing test → implement → refactor → THEN claim complete.
