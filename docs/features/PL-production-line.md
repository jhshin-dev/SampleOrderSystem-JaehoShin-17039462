...
| ID | 기능 | 진입점 |
|----|------|--------|
| PL-01 | 생산 현황 조회 | 생산 담당자 메뉴 → 생산 라인 → 1 |
| PL-02 | 생산 대기 큐 조회 | 생산 담당자 메뉴 → 생산 라인 → 2 |
| PL-03 | 생산 완료 처리 | 생산 담당자 메뉴 → 생산 라인 → 3 |
...
> 대기 큐 = `PRODUCING` 상태 주문을 `updatedAt` 오름차순으로 정렬한 목록
> (`updatedAt` = PRODUCING 상태로 전이된 시각 = 생산 라인 등록 시각)
...
| 고객명 | customerName |
| 수량 | quantity |
| 등록일시 | updatedAt (PRODUCING 전이 시각) |

### 처리 흐름

```
OrderRepository::findAll() → status == PRODUCING 필터 → updatedAt 오름차순 정렬
    │
    ├─ 결과 있음 → 순번과 함께 테이블 출력
...
생산이 완료되면 주문 상태를 `PRODUCING → CONFIRMED`로 자동 전이한다.

```
주문 ID 입력 → PRODUCING 상태 확인
    │
    └─ OrderRepository::updateStatus(id, CONFIRMED)
           └─ updatedAt 갱신
           └─ Sample.stock += 실 생산량  (생산된 수량만큼 재고 증가)
